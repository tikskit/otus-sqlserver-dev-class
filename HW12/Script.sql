...
IF OBJECT_ID('Sales.GetBestBuyer') IS NOT NULL
	DROP FUNCTION Sales.GetBestBuyer;
GO

CREATE FUNCTION Sales.GetBestBuyer()
RETURNS INT
AS
BEGIN
	...
	*/
	...
	SELECT TOP 1 @CustometID = Customers.CustomerID--, Customers.CustomerName, PurchaseSum.PurchaseValue
	FROM Sales.Invoices 
...
BEGIN
	...
	SELECT SUM(InvoiceLines.Quantity * ISNULL(InvoiceLines.UnitPrice, StockItems.UnitPrice)) AS Total
	FROM Sales.InvoiceLines
	INNER JOIN Warehouse.StockItems ON Warehouse.StockItems.StockItemID=InvoiceLines.StockItemID
...
-- Пример вызова:
EXEC Sales.GetCustomerPurchasesTotal 834
...
EXEC Sales.GetCustomerPurchasesTotal @CustomerID;
